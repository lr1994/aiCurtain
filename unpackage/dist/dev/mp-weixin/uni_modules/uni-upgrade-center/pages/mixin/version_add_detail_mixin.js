...
function getValidator(fields2) {
  let reuslt = {};
  for (let key in uni_modules_uniUpgradeCenter_js_sdk_validator_opendbAppVersions.validator) {
    if (fields2.includes(key)) {
      reuslt[key] = uni_modules_uniUpgradeCenter_js_sdk_validator_opendbAppVersions.validator[key];
    }
  }
  return reuslt;
}
const fields = "appid,name,title,contents,platform,type,version,min_uni_version,url,stable_publish,is_silently,is_mandatory,create_date";
const addAndDetail = {
  data() {
    return {
      labelWidth: "80px",
      ...
      appFileList: null,
      // 上传包
      type_valuetotext: uni_modules_uniUpgradeCenter_js_sdk_validator_opendbAppVersions.enumConverter.type_valuetotext,
      preUrl: "",
      formData: {
        "appid": "",
        "name": "",
