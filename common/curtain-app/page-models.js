...
function normalizeWorkRecord(item = {}) {
...
function createWorkStatusTabs(list = []) {
	const counters = {
		all: list.length,
		processing: 0,
		success: 0,
...
function filterWorksByStatus(list = [], status = 'all') {
...
function createShowroomCategories(list = []) {
	const categories = [
		{ key: 'all', label: '全部' }
	];
	const usedKeys = {
...
function buildRenderHomeViewModel({
	hasLogin = false,
	balance = 0,
	generating = false,
	canGenerate = false,
